...
public class GunScript : MonoBehaviour {
	...
	*/
	void FixedUpdate(){
		 ...
		 */
		//if aiming
		if(ControlFreak2.CF2Input.GetAxis("Fire2") != 0 && !reloading && !meeleAttack){
	...
	void CrossHairExpansionWhenWalking(){
		...
		if(player.GetComponent<Rigidbody>().velocity.magnitude > 1 && ControlFreak2.CF2Input.GetAxis("Fire1") == 0){//ifnot shooting
	...
	void Sprint(){// Running();  so i can find it with CTRL + F
		if (ControlFreak2.CF2Input.GetAxis ("Vertical") > 0 && ControlFreak2.CF2Input.GetAxisRaw ("Fire2") == 0 && meeleAttack == false && ControlFreak2.CF2Input.GetAxisRaw ("Fire1") == 0) {
	...
	*/
	void MeeleAttack(){	
		...
		if(ControlFreak2.CF2Input.GetKeyDown(KeyCode.Q) && !meeleAttack){			
	...
	private void ShootMethod(){
		if(waitTillNextFire <= 0 && !reloading && pmS.maxSpeed < 5){
			...
			if(bulletsInTheGun > 0){
				...
				if (Physics.SphereCast(bulletSpawnPlace.transform.position, 0.1f, bulletSpawnPlace.transform.forward,
					    out RaycastHit hit, 3000, -1, QueryTriggerInteraction.Ignore)) {
					hit.collider.BroadcastMessage("Damege",50);
				}
	...
	*/
	...
	void DrawCrosshair(){
		...
		if(ControlFreak2.CF2Input.GetAxis("Fire2") == 0){//if not aiming draw
	...
	*/
	void Animations(){
		...
		if(handsAnimator){
			...
			handsAnimator.SetBool("aiming", ControlFreak2.CF2Input.GetButton("Fire2"));
			handsAnimator.SetInteger("maxSpeed", pmS.maxSpeed);
			if(ControlFreak2.CF2Input.GetKeyDown(KeyCode.R) && pmS.maxSpeed < 5 && !reloading && !meeleAttack/* && !aiming*/){
				StartCoroutine("Reload_Animation");
			}
		}
